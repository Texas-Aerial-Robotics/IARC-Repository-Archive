...
void drawObject(vector<Roomba> theRoombas, Mat &frame, int type) {
	...
	if (type == 3) {
		for (int i = 0; i < theRoombas.size(); i++) {
			...
			cv::circle(frame, cv::Point(theRoombas.at(i).getXPos(), theRoombas.at(i).getYPos()), 10, cv::Scalar(0, 0, 255));
			cout << "Grid square # " << i << " x Pos   " << theRoombas.at(i).getXPos() << "  Y Pos    " << theRoombas.at(i).getYPos() << endl;
			cv::putText(frame, intToString(theRoombas.at(i).getXPos()) + " , " + intToString(theRoombas.at(i).getYPos()), cv::Point(theRoombas.at(i).getXPos(), theRoombas.at(i).getYPos() + 20), 1, 1, Scalar(0, 255, 0));
		}
...
int main(int argc, char* argv[])
{
	...
	while (1) {
		...
		{
			//hardcode colors
			Roomba redone, greenone,gridsquare;
			...
			gridsquare.setHSVmin(Scalar(56, 0, 0));
			gridsquare.setHSVmax(Scalar(256, 212, 101));
			gridsquare.settype(3);
			...
			cvtColor(cameraFeed, HSV, COLOR_BGR2HSV);
			inRange(HSV, gridsquare.getHSVmin(), gridsquare.getHSVmax(), threshold);
			morphOps(threshold);
			trackFilteredObject(threshold, HSV, cameraFeed, gridsquare.gettype());
		}

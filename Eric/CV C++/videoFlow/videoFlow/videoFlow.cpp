// videoFlow.cpp : Defines the entry point for the console application.
//
...
int main()
{
	...
	Point2f flow;
	Point2f flowSum;
	Point2f POS1 = Point2f(500*5, 500*5);
	Point2f POS2;
	...
	while (1)
	{
		...
		if (counter != 1)
		{
			...
			flow.y = -1*flowSum.y / cornersFound;
			
			POS2 = POS1 + flow;
			cout << "X " << (POS2.x*h*.75)/640 << " Y " << (POS2.y*h*.55)/480  << endl;
			line(pos, POS1/5, POS2/5, Scalar(0, 255, 0), 2, 8);
			POS1 = POS2;
			imshow("Image A", imgA);
			imshow("Image B", imgB);
			imshow("flow", imgC);
